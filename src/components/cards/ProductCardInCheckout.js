...
const ProductCardInCheckout = ({p}) => {
    ...
    const handleRemove = () => {
        ...
        if (typeof window !== undefined) {
            ...
            // [1,2,3,4,5] finds the index, and remove the given number of elements starting from that index
            cart.map((product, i) => {
                if (product._id === p._id) {
                    cart.splice(i, 1)
                }
    ...
    return (
        <tbody>
            <tr>
                ...
                <td className="text-center">
                    {p.shipping === "Yes" ? <CheckCircleOutlined className="text-success"/> : <CloseCircleOutlined className="texr-danger"/>}
                </td>
                <td className="text-center">
